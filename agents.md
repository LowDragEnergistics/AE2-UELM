...
3. **修改位置**：`gradle.properties`（`version=x.y.z`）与 `src/main/resources/META-INF/mods.toml`
   （`version="x.y.z"`——构建时由 processResources 以工程版本替换，源文件需保持一致）。
4. **git tag**：按仓库约定使用 `forge/v<版本>` 格式（如 `forge/v0.1.1`）；tag 打在版本提升提交上，随分支一起推送。
5. **CI 一致性**：版本提升提交后，`TunnelPattern CI` 与 `Build and Test` 须全绿；版本不影响 datagen（`runData` 后 generated 无 diff）。
6. **默认分支同步**：版本提升提交应通过 PR 合入 `forge/1.20.1`，保持默认分支版本与功能/修复一致。
